...
3. **Ordem de empilhamento (z-index) é por categoria, não por arquivo**: `body_types` (base) → `faces` → `clothes` → demais categorias futuras (`hairType`, `eyeType`, `accessory`, `top`/`pants`/`shoes` quando os PNGs existirem). A ordem exata de z-index entre as categorias ainda não foi validada visualmente (ex.: cabelo provavelmente por cima do corpo mas por trás ou na frente do rosto, a depender do estilo) — decidir quando os PNGs de cabelo/olho chegarem.
...
- **`clothes/0.png` é uma peça única (short)**, mas `Appearance` modela `top` e `pants` como campos independentes (guarda-roupa trocável separadamente, ver item 7 de [[0019-personagem-montado-em-camadas-com-rig-2d]]). Precisa decidir se `clothes` vira duas pastas (`tops/`, `pants/`) ou se o short atual é só um placeholder de corpo inteiro até a arte de guarda-roupa separada chegar.
...
- **`accessory` e `shoes` também não têm nenhum PNG** — esses campos de `Appearance` continuam sem asset real (ficam no valor default `"default"` / `null`).
