...
9. **Skills são um catálogo dinâmico, não um enum fixo**: `Character.skills` é um mapa livre `{ [skillId]: pontos }`, validado contra `SKILL_DEFINITIONS` (lista de `{ id, label }` em `domain/entities/skill.ts`). Adicionar uma skill nova é só acrescentar uma entrada nessa lista — não exige migração de schema nem mudança de tipo. As skills reais ainda não foram definidas pelo game design (ver docs/game-design/skills-and-study.md) — o catálogo atual é só um placeholder. ~~Na criação, o jogador distribui um orçamento fixo de 4 pontos entre as skills do catálogo (`INITIAL_SKILL_POINTS_BUDGET`)~~ — **superado por [[0024-personagem-nasce-sem-skills]]**: não existe mais alocação inicial, todo personagem nasce com `skills: {}` (nível 0 em tudo).
