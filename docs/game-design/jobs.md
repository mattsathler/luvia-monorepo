...
- **Habilidade Principal / Secundária / Terciária**: cada Cargo (não o Prédio inteiro) define até 3 skills do catálogo existente (`SKILL_DEFINITIONS`) que importam pra ele, em ordem de peso — principal obrigatória, secundária e terciária opcionais. Cargos diferentes do mesmo prédio podem exigir skills diferentes (ex.: "Atendente" pesa Carisma; "Coordenador" do mesmo prédio pesa Inteligência). Essa skill pondera eficiência (ganho e velocidade de promoção) — **não** é um requisito rígido de contratação: o cargo de entrada de todo prédio é sempre contratável, mesmo com skill em nível 0 (ver "Vagas" abaixo, e docs/decisions/0024-personagem-nasce-sem-skills.md — não existe ainda nenhum mecanismo de crescimento de skill implementado, então um requisito rígido travaria a carreira pra sempre).

### Vagas

O cargo de entrada (índice 0) de todo prédio é **sempre disponível**, sem limite — é o que garante a garantia de "sempre disponível" da seção acima. Cargos de promoção acima dele têm um número limitado de vagas (`vacancySlots` no catálogo), por **instância física** de prédio (`Workplace` — lembrar que prédios obrigatórios são replicados por Bairro, então cada Bairro tem sua própria contagem de vagas, independente dos demais). Se todas as vagas de um cargo estiverem ocupadas por outros personagens quando alguém cruza o `scoreToPromote`, a promoção não acontece ainda — o personagem fica **represado** no cargo atual, sem perder o score acumulado, e é promovido assim que uma vaga abrir (alguém sair daquele cargo por promoção ou troca de emprego). Cargo final de cada hierarquia tem sempre 1 vaga (só um chefe por prédio).
...
- Quando o score acumulado cruza o threshold, o personagem é promovido automaticamente pro próximo cargo da hierarquia (score zera e recomeça no cargo novo) — verificado no mesmo recompute/tick que já processa os outros efeitos de atividade (ver [[../technical/simulation-tick]]), sem precisar de um scheduler novo, **desde que haja vaga livre no próximo cargo** (ver "Vagas" acima) — sem vaga, o score cruzado é preservado e a checagem se repete no próximo recompute.
