...
describe('applyActivityEffect', () => {
  it('returns the same stats unchanged when elapsedMs is zero or negative', () => {
    expect(applyActivityEffect(stats, 'idle', 0)).toBe(stats);
    expect(applyActivityEffect(stats, 'idle', -1000)).toBe(stats);
  });

  it('applies the idle rate over a positive elapsed interval', () => {
    const result = applyActivityEffect(stats, 'idle', 60_000);
