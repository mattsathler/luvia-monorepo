...
export type CityView = { width: number; height: number; backgroundColor: string };

/**
 * Ver docs/decisions/0005-cidade-unica-persistente.md — existe apenas uma
 * cidade, então esta leitura não recebe nenhum id.
 *
 * Só as dimensões e a cor de fundo — não o terreno nem os lotes (isso é
 * `GetCityChunkUseCase`, pedido por chunk sob demanda). Ver
 * docs/technical/lowys-carregamento-em-chunks.md.
 */
@Injectable()
export class GetCityUseCase {
  ...
  async execute(): Promise<CityView> {
    ...
    return { width: cityMap.width, height: cityMap.height, backgroundColor: cityMap.backgroundColor };
  }
}
